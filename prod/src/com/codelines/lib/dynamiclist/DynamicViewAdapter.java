...
public abstract class DynamicViewAdapter<T> extends BaseAdapter
{
    protected final Context _context;

    private final ArrayList<AdapterItem<T>> _list;
    ...
    public DynamicViewAdapter(Context context, int progressResourceId, int unavailableResourceId, int emptyResourceId)
    {
        ...
        _list = new ArrayList<AdapterItem<T>>();
    ...
    protected abstract View onDataView(int position, AdapterItem<T> item, View convertView, ViewGroup parent);
    ...
    protected void addItem(T value, Object tag)
    {
        AdapterItem<T> item = new AdapterItem<T>(value, tag);
        _list.add(item);
    }
    ...
    private void tryShowItem(ViewType type)
    {
        ...
        {
            _list.add(new AdapterItem<T>(type));
        }
